...
public class FFT2D {
  ...
  protected void checkData(double data[], int rowspan) {
    if(rowspan<2*ncols) {
      throw new IllegalArgumentException("The row span "+rowspan+"is shorter than the row length "+2*ncols);
    }
    if(nrows*rowspan>data.length) {
      throw new IllegalArgumentException("The data array is too small for "+nrows+"x"+rowspan+" data.length="+data.length);
    }
  }
