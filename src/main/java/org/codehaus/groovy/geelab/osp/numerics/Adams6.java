    ...
    if (counter<5) { // Use Fehlberg8 to start the method
      ...
      counter++;
        ...
        for (int i = 0; i < numEqn; i++) { // Predictor
            temp_state[i] = state[i] + stepSize*(4277*fn[i] - 7923*fn1[i] + 9982*fn2[i] - 7298*fn3[i]
                                               + 2877*fn4[i] - 475*fn5[i])/720;
        }
        ode.getRate(temp_state, temp_rate);
        for (int i = 0; i < numEqn; i++) { // Corrector
            state[i] = state[i] + stepSize*(475*temp_rate[i]  + 1427*fn[i] - 798*fn1[i] + 482*fn2[i] -173*fn3[i]
                                           + 27*fn4[i])/1440;
        }
    }
